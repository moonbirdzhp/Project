...
## 编译和链接参数设定. 无论C还是C++都使用CFLAGS, 即CPPFLAGS or CXXFLAGS没有被
## 使用. 注意: 这样是不支持C和C++或其他语言混编的, 当然C代码可以当成C++来编译. 
## 如果是个子项目, 使用父项目的编译参数, 注释掉相应的行. 如果要在父项目的编译
## 参数的基础上增加或覆盖, 使用'+=', 如"CFLAGS += $(CFLAGS) YOUR_OWN_FLAGS".
## 如果不想使用父项目或环境变量的参数, 使用如 "CFLAGS := "这样的方式. CFLAGS
## 用于release编译参数; DCFLAGS用于debug编译参数.
CFLAGS		:= $(CFLAGS_PASS)
